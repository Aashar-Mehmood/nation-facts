...
export default function CountryCard(props) {
  const {capital, flags:{png, alt}, name:{official}, population, region} = props;
  return (
    <div className="mt-14 p-1 rounded-md shadow bg-lightElementBg text-lightText dark:bg-darkElementBg dark:text-darkText">
      ...
      <div className="px-6 py-10">
        ...
        <Link to={`/details/${official}`} className="inline-block mt-6 text-indigo-600 border border-indigo-600 rounded-md px-4 py-2">
          Details
          <FontAwesomeIcon className="ml-2 relative top-0.5 rotate-90" icon={faArrowUp}></FontAwesomeIcon>
        </Link>
      </div>
    </div>
  );
