  ...
  return loading ? (
    ...
    <Fragment>
      <div className="mt-4">
        <button
          onClick={() => navigate("/")}
          className="px-6 py-2 rounded-md shadow bg-lightElementBg dark:bg-darkElementBg"
        >
      ...
      <div className="md:flex md:flex-wrap md:justify-between">
        {details.map((detail) => {
          return (
            <div
              key={nanoid(10)}
              className="mt-10 p-1 rounded-md shadow-lg bg-lightElementBg dark:bg-darkElementBg md:w-43% lg:w-28%"
            >
              <img
                className="rounded-md border w-full border-slate-900 lg:aspect-video"
              ...
              <div className="p-4 sm:p-8">
                ...
                <p className="mb-10">
                  ...
                  {Object.values(detail.currencies).map((currency) => {
                    return currency.name + " ";
                  })}
                </p>
                <h2 className="text-2xl mb-2 font-bold">Languages : </h2>
                {Object.values(detail.languages).map((lang) => {
                  return (
                    <div
                      key={nanoid(10)}
                      className="inline-block border px-4 py-1 shadow mr-2 my-1"
                    >
                      {lang}
                    </div>
                  );
